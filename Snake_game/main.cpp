...
extern bool enigma;
bool gameOver = false;
double speed = 100; // The greater, the slower
...
void display_callBack() {
	...
	if (gameOver) {
		char scores[10];
		_itoa_s(score, scores, 10); // Convert (int) score into string type based 10
		char notify[50] = "Your score: ";
		strcat_s(notify, scores); //Concatenate score into the string
